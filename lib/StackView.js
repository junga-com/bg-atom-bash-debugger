...
class BGDBStackFrame extends Component {
	constructor(frmNum, stackFrame, isSelected, ...p) {
		...
		this.mount([
			...
			new Component('caller:$td.caller ',   {
				content:'@TEXT'+this.frm.caller,
				dblclick: ()=>{this.runToThisFrame()}
			}),
		...
		// fill in cmdLine from source file if needed
		...
		// set a tooltip to show the frmNum
		this.caller.addTooltip("double click to run to this frame ("+this.frmNum+")");
