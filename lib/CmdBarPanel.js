...
class SwitchDebuggerButton extends Button {
	constructor(...p) {
		super(...p,  'btnResume:icon-playback-fast-forward');
		this.addTooltip("cycle through current debugging sessions", 'bg-atom-bash-debugger:stepOut');
	}
	onActivated() {
		atom.commands.dispatch(this.el, 'bg-atom-bash-debugger:cycleDebugger');
	}
}
