...
class BGBashDebugger extends BGAtomPlugin {
	constructor(state) {
		...
		this.addCommand("bg-atom-bash-debugger:cycleDebugger", ()=>this.dispatchRunCommand('cycleDebugger'));
	...
	switchTo(debuggerID)
	{
		if (debuggerID && ! this.debuggers.has(debuggerID))
			...
			console.warn("bg-bash-debugger.switchTo(debuggerID='"+debuggerID+"'): there is only one running debuggers");
		else {
			if (debuggerID) {
