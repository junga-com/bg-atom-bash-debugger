...
class BGBashDebugger extends BGAtomPlugin {
	...
	addDebuggedProcess(theDebugger) {
		...
		this.debuggers.set(theDebugger.pid, this.activeDebugger);
		this.onDepChanged();
	...
	removeDebuggedProcess(pid) {
		if (this.debuggers.has(pid)) {
			var temp = this.debuggers.get(pid);
			this.debuggers.delete(pid);
			if (temp)
				temp.destroy();
		}
		if (this.activeDebugger && ! this.debuggers.has(this.activeDebugger.pid) )
			this.activeDebugger = null;
		this.getActiveDebugger();
		this.onDepChanged();
		if (!this.activeDebugger)
