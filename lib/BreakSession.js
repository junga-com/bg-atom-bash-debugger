...
//		this.showLocationInSource(breakLocation.file, breakLocation.line, breakLocation.cmd);
	...
	async showLocationInSource(file, line, cmd) {
		if  ((!/[<]handler[>]$/.test(file)) && !fs.existsSync(file)) {
			atom.notifications.addWarning("The source file '"+file+"' can not be found to show this stack frame position");
			return;
		}

		var match, sig, pid
		if (match = /^(?<sig>[^-]*)-(?<pid>.*)[<]handler[>]/.exec(file)) {
			({sig,pid} = match.groups);
			var tmpDir=fs.mkdtempSync("bg_atom_dbg_"+sig+"_");
			file = path.join(tmpDir,sig+"_handler");
			if (sig == "<UNK>") {
				fs.writeFileSync(file, "A trap has been invoked but we do not yet know which trap pid='"+pid+"'");
			}
			else if (this.traps[sig]) {
				fs.writeFileSync(file, this.traps[sig]);
			}
			else {
				fs.writeFileSync(file, "hmm... this should not happen. we dont have the source for the signal handler for '"+sig+"'  pid='"+pid+"'");
			}
		}
		...
		if (!this.srcMarkers.has(file+":"+line))
	...
	setStack(stack)
	{
		...
		while (this.currentFrame<this.stack.length
			&& (!/_HANDLER$/.test(this.stack[this.currentFrame].caller))
