...
export class BGBashDebuggerTutorial extends Tutorial {
	static configure(configKey) {
		BGBashDebuggerTutorial.configKey = configKey;
		atom.config.addDep(BGBashDebuggerTutorial.configKey, BGBashDebuggerTutorial)

		BGBashDebuggerTutorial.onConfigChanged({});
	}

	static onConfigChanged({key, newValue}) {
		if (!newValue) newValue=atom.config.get(BGBashDebuggerTutorial.configKey);
		if (newValue || Tutorial.resumeStateExists()) {
			if (newValue) atom.config.set(BGBashDebuggerTutorial.configKey, false);
			if (!BGBashDebuggerTutorial.instance)
				BGBashDebuggerTutorial.instance = new BGBashDebuggerTutorial();
		}
	}
	...
	page4() {
		...
		this.dialog.dismiss();
		BGBashDebuggerTutorial.instance = null;
		this.dialog = atom.notifications.addInfo(dedent`
